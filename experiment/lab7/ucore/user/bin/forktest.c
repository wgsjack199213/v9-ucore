...
umain(void) {
    ...
    for (n = 0; n < max_child; n ++) {
        ...
        do { if (!(pid > 0)) { __panic("ucore/user/forktest.c", 15, "assertion failed: %s", "pid > 0"); } } while (0);
    }

    if (n > max_child) {
        __panic("ucore/user/forktest.c", 19, "fork claimed to work %d times!\n", n);
    }

    for (; n > 0; n --) {
        if (wait() != 0) {
            __panic("ucore/user/forktest.c", 24, "wait stopped early\n");
        }
    }

    if (wait() == 0) {
        __panic("ucore/user/forktest.c", 29, "wait got too many\n");
    }
